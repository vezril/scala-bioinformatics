...
import bio.algorithms.graph.GenomeAssemblyReads
...
object GASMProb {
  ...
  private val DataPath =
    "/Users/cference/Code/scala-bioinformatics/src/main/scala/resources/gasm_data.txt"
    ...
    for {
      ...
      result = for {
        reads <- lines
          ...
          .map(_.toVector)
        problem <- GenomeAssemblyReadsProblem.from(reads).left.map(_.toString)
      } yield GenomeAssemblyReads.assemble(problem)
      _ <- result match {
