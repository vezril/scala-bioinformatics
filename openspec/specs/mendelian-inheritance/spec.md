## Purpose
Define the Mendelian inheritance capability that computes the probability that two randomly selected mating organisms from a population of known genotypes produce offspring displaying the dominant phenotype. This is Rosalind problem IPRB. The capability provides a `Genotype` ADT, a validated `Population` of genotype counts, a validated `Probability` value in `[0, 1]`, and a pure analytical function that computes the dominant-phenotype probability without Monte Carlo simulation.
