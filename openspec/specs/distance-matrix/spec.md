...
Computes the `n × n` p-distance matrix of a collection of equal-length DNA
strings (Rosalind spec — PDST, "Creating a Distance Matrix"). Each entry
`D[i][j]` is the p-distance between strings `i` and `j` — the fraction of
positions at which they differ. Provides a validated `DistanceMatrixProblem`
input bundle (at most 10 strings, each length at most 1000, all equal length),
a `DistanceMatrix` result type that formats to 5 decimal places, and the pure
`PDistanceMatrix.compute` algorithm.
