## Purpose
Define the wright-fisher-genetic-drift capability: the `WrightFisherProblem` validated parameter bundle (four positive integers `n, m, g, k` with bounds `1 ≤ n ≤ 7`, `1 ≤ m ≤ 2n`, `1 ≤ g ≤ 6`, `1 ≤ k ≤ 2n`) and its 8-case `WrightFisherProblemError` ADT, and the `WrightFisher.atLeast(problem): Probability` algorithm computing the probability that after `g` generations of the Wright-Fisher Markov chain (starting from `m` dominant alleles in a `2n`-chromosome population) at least `k` recessive alleles are present. Serves the Rosalind "The Wright-Fisher Model of Genetic Drift" (WFMD) problem and is the sixth algorithm in the `genetics` subdomain — the framework's first Markov-chain computation.
