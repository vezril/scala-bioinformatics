## Purpose
Define the independent-chromosome-segregation capability: the `ChromosomePairs` validated value-type wrapper enforcing `1 ≤ n ≤ 50` (where `n` is the haploid chromosome-pair count and the total chromosome count modeled is `2n`), its `ChromosomePairsError` ADT, and the `IndependentSegregation.logProbs(pairs): Vector[Double]` algorithm computing `log10(P(X ≥ k))` for `k = 1..2n` under `X ~ Bin(2n, 1/2)` via a PMF multiplicative recurrence and right-to-left upper-tail sweep. Serves the Rosalind "Independent Segregation of Chromosomes" (INDC) problem and is the fourth algorithm in the `genetics` subdomain alongside `MendelianInheritance`, `ExpectedOffspring`, and `IndependentAlleles`.
