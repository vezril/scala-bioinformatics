# shared-spliced-motif Specification

## Purpose

Finds a longest common subsequence (LCS) of two DNA strings — symbols in
order (not necessarily contiguous) that appear in both inputs (Rosalind
spec 39 — LCSQ, "Finding a Shared Spliced Motif"). Provides a validated
`SharedSplicedMotifProblem` input bundle (length caps on both strings) and
the `SharedSplicedMotif.find` classical `O(m · n)` DP + backtracking
algorithm returning one valid LCS as `String`.
