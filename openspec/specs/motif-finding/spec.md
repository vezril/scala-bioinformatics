## Purpose
Define the framework's first positional substring-search capability: `MotifLocations.find(text: DnaString, motif: DnaString): Vector[Int]`, returning every 1-based starting position where `motif` occurs as a contiguous substring of `text`. Overlapping matches are included. The function is total — every pair of `DnaString`s produces a defined `Vector[Int]`. Serves the Rosalind "Finding a Motif in DNA" problem and establishes the convention (1-based, overlapping, total, ascending) for every future positional sequence-search algorithm.
