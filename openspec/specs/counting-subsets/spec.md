## Purpose
Define the counting-subsets capability: the `SubsetUniverseSize` validated value-type wrapper enforcing `1 ≤ n ≤ 1000`, its `SubsetUniverseSizeError` ADT, and the `Subsets.count(size): Int` algorithm computing `2^n mod 1,000,000` via an incremental product with per-step modulo. Serves the Rosalind "Counting Subsets" (SSET) problem and is the third algorithm in the `combinatorics` subdomain alongside `Permutations` (spec 15) and `PartialPermutations` (spec 16), sharing the single-value-type input pattern with the former and the per-step-modulo computation pattern with the latter.
