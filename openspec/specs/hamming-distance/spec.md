## Purpose
Define the framework's first symmetric pairwise DNA comparison: `HammingDistance.between(a: DnaString, b: DnaString): Either[HammingError, Int]`, returning the number of positions at which two equal-length DNA sequences differ. Length-mismatched inputs return `Left(HammingError.LengthMismatch(...))` carrying both lengths. Two empty `DnaString`s have distance 0. Serves the Rosalind "Counting Point Mutations" problem and establishes the framework's distance-metric vocabulary for future approximate-matching, clustering, and SNP-counting algorithms.
