## Purpose
Define the framework's protein-layer vocabulary used by RNA translation and any downstream protein-aware algorithm: the `AminoAcid` ADT (20 standard amino acids with single-letter codes), the `Codon` triple-nucleotide type and its parsing helpers, the `CodonOutcome` ADT (amino acid product or Stop signal), and the `GeneticCode` lookup encapsulating the standard RNA codon table.
