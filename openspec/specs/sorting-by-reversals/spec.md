# sorting-by-reversals Specification

## Purpose

Sorts one permutation into another by contiguous-interval reversals, reporting
both the reversal distance and an explicit ordered collection of reversals that
realizes it (Rosalind spec — SORT, "Sorting by Reversals"). Reuses the
validated `ReversalDistanceProblem` input bundle, adds a `Reversal` interval
domain type and a `ReversalSorting` result type, and provides the
`ReversalSortingSearch.sort` algorithm that returns a minimal sequence of
reversals transforming the source permutation into the target.
