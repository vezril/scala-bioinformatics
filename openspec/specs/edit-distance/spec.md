# edit-distance Specification

## Purpose

Computes the Levenshtein (edit) distance between two protein strings —
the minimum number of single-symbol substitutions, insertions, and
deletions needed to transform `left` into `right` (Rosalind spec 40 —
EDIT, "Edit Distance"). Provides a validated `EditDistanceProblem` input
bundle (length caps on both strings) and the `EditDistance.compute`
classical `O(m · n)` DP algorithm returning the integer distance.
