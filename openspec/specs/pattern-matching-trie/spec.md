...
This capability constructs the trie (a symbol-labelled adjacency list) that encodes a collection of DNA patterns, with the root labelled `1` and new nodes numbered in edge-creation order as patterns are inserted (Rosalind TRIE).
