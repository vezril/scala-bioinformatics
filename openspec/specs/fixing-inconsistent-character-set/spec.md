...
Repairs an inconsistent `0/1` character table by deleting exactly one conflicting row so the
remaining characters' splits are mutually compatible, per the Rosalind "Fixing an Inconsistent
Character Set" (CSET) problem. Provides the validated `InconsistentCharacterSetProblem` input
bundle with its `InconsistentCharacterSetProblemError` ADT and the `ConsistentCharacterTable`
that renders the retained rows.
