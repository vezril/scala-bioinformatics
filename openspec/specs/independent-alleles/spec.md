## Purpose
Define the binomial-tail capability for Tom's family tree: the `IndependentAllelesProblem` validated parameter bundle (generations + atLeast + derived `populationSize: Long = 2^generations`), its error ADT, and the `IndependentAlleles.probability(problem): Probability` algorithm computing `P(X >= atLeast)` for `X ~ Binomial(2^generations, 1/4)`. Returns the framework's existing `Probability` value. Serves the Rosalind "Independent Alleles" problem, extending the genetics subdomain alongside spec-5 `MendelianInheritance` and spec-11 `ExpectedOffspring`.
