...
Computes the six derived sets — union, intersection, the two set differences, and the two
complements — over a universe `{1, …, n}` and two subsets `A` and `B`, per the Rosalind
"Introduction to Set Operations" (SETO) problem. Provides the validated
`SetOperationsProblem` input bundle with its `SetOperationsProblemError` ADT and the
`SetOperationsResult` that renders each derived set in ascending, brace-delimited form.
