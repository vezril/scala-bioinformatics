# reversing-substitutions Specification
...
Reversing Substitutions"). A reversing substitution is a pair of
parent-child edges `(s, t)` and `(v, w)` on a root-down path where
position `i` undergoes a substitution at `(s, t)` and is reverted to
its original value at `(v, w)`, with the substituted value preserved
along the intermediate sub-path from `t` to `v`. Provides a validated
`ReversingSubstitutionsProblem` input bundle, a `ReversingSubstitution`
output record, and the per-column tree-walk `ReversingSubstitutions.findAll`
algorithm.
