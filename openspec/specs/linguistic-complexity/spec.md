...
This capability computes the linguistic complexity `lc(s) = sub(s) / m(4,n)` of a DNA string — the ratio of its number of distinct substrings to the maximum possible number of distinct substrings (Rosalind LING) — using a suffix array plus LCP array.
