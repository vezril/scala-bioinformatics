...
Compute the probability that at least one of N random DNA strings of length |s|, generated under a given GC-content x, equals a given motif s (Rosalind RSTR). The per-symbol match probability yields a single-string match probability p = ∏ P(symbol | x), and the overall result is 1 - (1 - p)^N.
