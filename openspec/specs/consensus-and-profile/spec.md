## Purpose
Define the consensus-and-profile capability: the `ConsensusProfileProblem` validated input bundle (non-empty + equal-length cross-constraint) and its error ADT, the `ProfileMatrix` 4-row per-column-count matrix (indexed by `DnaNucleotide`, with `.a/.c/.g/.t/.width` accessors) and its error ADT, the `ConsensusProfileResult(consensus, profile)` pair, and the `ConsensusProfile.compute(problem): ConsensusProfileResult` algorithm computing both the profile matrix and an alphabetically-tie-broken consensus string. Serves the Rosalind "Consensus and Profile" problem and seeds the framework's `bio.{domain,algorithms}.matrix` subdomain — the home for matrix-shaped result types.
