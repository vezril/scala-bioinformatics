## Purpose
Define the random-string-matching capability: the `RandomMatchProblem` validated input bundle (a `DnaString` plus a `Vector[Probability]` of GC-content values, with Rosalind upper bounds `|dna| ≤ 100` and `|gcContents| ≤ 20`), its `RandomMatchProblemError` ADT, and the `RandomMatch.logProbabilities(problem): Vector[Double]` algorithm computing the `log10` of the probability that a random DNA string drawn under each given GC content matches the input sequence exactly. Serves the Rosalind "Introduction to Random Strings" (PROB) problem and extends the `analysis` subdomain with its first algorithm that combines a `DnaString` with a `Vector[Probability]` to produce log-probability output.
