# alignment-based-phylogeny Specification

## Purpose

Solves the classical *small-parsimony* problem on a rooted binary
phylogenetic tree (Rosalind spec 44 — ALPH, "Alignment-Based
Phylogeny"). Given a `NewickTree` with named species at the leaves and
internal nodes plus a multiple alignment of the leaves' DNA sequences
(with `-` gap symbols allowed), the system assigns DNA strings to every
internal node so that the total per-edge Hamming distance is minimised.
The algorithm is *Sankoff* applied per column over the 5-symbol
alphabet `{A, C, G, T, -}`. Provides a `NamedSequence` record, a
validated `AlignmentBasedPhylogenyProblem` input bundle, an
`AlignmentBasedPhylogeny` output ADT, and the
`AlignmentBasedPhylogeny.solve` algorithm.
