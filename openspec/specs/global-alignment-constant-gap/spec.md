# global-alignment-constant-gap Specification

## Purpose

Computes the maximum global alignment score between two protein strings
under the BLOSUM62 substitution matrix with a **constant** gap penalty
(Rosalind spec 51 — GCON, "Global Alignment with Constant Gap Penalty").
Every gap — a maximal run of contiguous insertions or contiguous
deletions — is charged a flat `5` exactly once, regardless of its length.
This is the third gap model in the alignment family, after the *linear*
penalty of GLOB (spec 42) and the edit-distance unit gap (EDIT), and
introduces the multi-state (Gotoh-style) DP needed for any length-aware
gap model — the foundation for affine gaps. Provides a validated
`ConstantGapAlignmentScoreProblem` input bundle (length caps on both
strings), a `ConstantGapAlignmentScoreProblemError` ADT, and the
`ConstantGapAlignmentScore.compute` algorithm returning the integer
maximum score.
