## Purpose
Define the GC-content capability: a validated percentage domain type (`GcContent`), its error ADT (`GcContentError`), a total smart factory `GcContent.of(dna: DnaString)` computing GC content, and the `HighestGc.find` algorithm that identifies the `FastaRecord` with the maximum GC content in a list. Serves the Rosalind "Identifying Unknown DNA Quickly" problem and any downstream analysis that ranks sequences by GC composition.
