# read-error-correction Specification

## Purpose

Corrects single-nucleotide read errors in a collection of equal-length DNA reads
(Rosalind spec — CORR, "Error Correction in Reads"). A read is *correct* if it or
its reverse complement appears in the dataset at least twice, and *incorrect* if
it appears exactly once; each incorrect read is mapped to a correct read (or the
reverse complement of one) at Hamming distance exactly 1. Provides a validated
`ReadCorrectionProblem` input bundle (at most 1000 reads, each length at most 50,
all equal length), `Correction` and `ReadCorrections` result types, and the pure
`ReadErrorCorrection.correct` algorithm.
