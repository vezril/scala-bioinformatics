...
Validates a pair of DNA strings `s` and `t` and computes their optimal overlap
alignment — the highest-scoring alignment of a suffix of `s` against a prefix of `t`
(Rosalind OAP). Scoring is a match `+1`, a substitution `-2`, and each gap `-2`, and
the capability returns the optimal score together with the two aligned strings.
