...
Constructs the de Bruijn graph over a collection of (k+1)-mers and their reverse
complements, per the Rosalind "Constructing a De Bruijn Graph" (DBRU) problem.
Provides the validated `DeBruijnGraphProblem` input bundle with its
`DeBruijnGraphProblemError` ADT and the `DeBruijnGraph` result whose directed edges
are rendered one parenthesized `(from, to)` pair per line in lexicographic order.
