...
Construct the suffix tree of `s$` for a DNA string `s` and return the substrings of `s$` that label its edges (Rosalind SUFF), built by naive suffix insertion with edge splitting.
