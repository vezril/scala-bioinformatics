...
Enumerates every length-n string over an ordered alphabet in lexicographic order, per the
Rosalind "Enumerating k-mers Lexicographically" (LEXF) problem. Provides the validated
`KmerEnumerationProblem` input bundle with its `KmerEnumerationProblemError` ADT and the
`KmerEnumeration` result that renders the enumerated k-mers one per line.
