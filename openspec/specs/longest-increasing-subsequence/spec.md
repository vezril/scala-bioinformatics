# longest-increasing-subsequence Specification

## Purpose

Computes a longest strictly-increasing subsequence and a longest
strictly-decreasing subsequence of a permutation of `{1, …, n}` (Rosalind
spec — LGIS, "Longest Increasing Subsequence"). Provides a validated
`Permutation` domain type, a `PermutationError` ADT, a `MonotonicSubsequences`
result type, and the `LongestSubsequences.find` `O(n log n)` algorithm.
