# global-alignment-affine-gap Specification

## Purpose

Computes the maximum global alignment score between two protein strings
under the BLOSUM62 substitution matrix with an **affine** gap penalty
(Rosalind spec 52 — GAFF, "Global Alignment with Scoring Matrix and Affine
Gap Penalty"). A gap of length `L` costs `a + b·(L − 1)`, charging a
gap-opening penalty `a = 11` once and a gap-extension penalty `b = 1` for
each further symbol. This is the natural generalisation of the *linear*
gap of GLOB (spec 42) and the *constant* gap of GCON (spec 51). Unlike
those, GAFF must also return *one optimal alignment* (two augmented
strings), exercising traceback through the three-state (Gotoh) DP.
Provides a validated `AffineGapAlignmentProblem` input bundle (length caps
on both strings), an `AffineGapAlignmentProblemError` ADT, an
`AffineGapAlignment` result ADT (score plus two augmented alignment
strings), and the `AffineGapAlignment.compute` algorithm.
