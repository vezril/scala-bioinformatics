# disjoint-motifs-interweaving Specification

## Purpose

Determine, for every ordered pair of DNA patterns, whether some contiguous
substring of a text is an interleaving of the two patterns — that is, the two
patterns appear as disjoint subsequences that together cover the substring
exactly (Rosalind ITWV, "Finding Disjoint Motifs in a Gene"). Provides a
validated `InterwovenMotifProblem` input bundle, an `InterwovenMotifMatrix`
result type rendering the symmetric `n × n` 0/1 matrix, and an interleaving
