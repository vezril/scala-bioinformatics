# isolating-symbols-in-alignments Specification

## Purpose

Compute, for two DNA strings, the maximum global alignment score under the
+1/-1 mismatch score and the sum of every element of the symbol-isolation
matrix `M`, where `M[j][k]` is the maximum score of any global alignment
that aligns `left[j]` with `right[k]` (Rosalind OSYM — "Isolating Symbols
in Alignments").
