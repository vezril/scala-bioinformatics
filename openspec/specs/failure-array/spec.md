# failure-array Specification

## Purpose

Computes the Knuth-Morris-Pratt failure array (longest proper prefix-suffix
table) for a DNA string (Rosalind problem 31 — KMP, "Speeding Up Motif
Finding"). Provides a validated `FailureArrayProblem` input bundle and the
`FailureArray.compute` O(n) algorithm.
