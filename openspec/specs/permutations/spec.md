## Purpose
Define the framework's first combinatorial enumeration capability: the `PermutationLength` validated input type (bounded `[1, 7]` to match Rosalind and the factorial blow-up), the `PermutationLengthError` ADT, and the `Permutations.enumerate(length): Vector[Vector[Int]]` algorithm producing every permutation of `{1, ..., length.value}`. Establishes the `bio.{domain,algorithms}.combinatorics` subdomain — pure-math primitives that don't belong inside a biology-named folder. Serves the Rosalind "Enumerating Gene Orders" problem.
