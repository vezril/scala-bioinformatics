# shared-motif Specification

## Purpose

Finds a longest common substring of a collection of DNA strings (Rosalind
spec 38 — LCSM, "Finding a Shared Motif"). Returns the
lexicographically-smallest substring of maximum length that appears in every
input. Provides a validated `SharedMotifProblem` input bundle
(collection-size and per-string length caps) and the `SharedMotif.find`
`O(k · n · log n)` binary-search + set-intersection algorithm.
