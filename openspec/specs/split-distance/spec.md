...
Computes the split distance d_split(T1,T2) = 2(n−3) − 2·s between two unrooted
binary trees on the same taxa per the Rosalind "Split Distance" (SPTD) problem.
Provides the validated `SplitDistanceProblem` two-tree input bundle with its
`SplitDistanceProblemError` ADT, the canonicalised `Split` ADT representing a
bipartition of the taxa, and the `SplitDistance.compute` algorithm that counts
the nontrivial splits shared by both trees.
