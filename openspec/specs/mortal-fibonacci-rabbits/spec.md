## Purpose
Define the mortal-Fibonacci-rabbits capability: the `MortalRabbitProblem` validated parameter bundle (months + lifespan), its error ADT, and the `MortalFibonacciRabbits.population(problem): BigInt` algorithm that simulates a rabbit population under the standard 1-offspring rule with a fixed `lifespan`-month lifetime per pair. Serves the Rosalind "Mortal Fibonacci Rabbits" problem and coexists with the spec-6 immortal generalized-Fibonacci recurrence as a distinct parameter+algorithm pair under `bio.{domain,algorithms}.recurrence`.
