## Purpose
Define the alternative-splicing-counting capability: the `CombinationSumProblem` validated parameter bundle (two integers with cross-constraint `0 ≤ m ≤ n ≤ 2000`) and its error ADT (`NegativeN`, `NExceedsMaximum`, `NegativeM`, `MExceedsN`), and the `Combinations.sumFrom(problem): Int` algorithm computing `Σ_{k=m}^{n} C(n, k) mod 1,000,000` via a single modular Pascal's-triangle row build. Serves the Rosalind "Introduction to Alternative Splicing" (ASPC) problem and is the fourth algorithm in the `combinatorics` subdomain — the first one where the integer parameters' lower bound is `0` rather than `1`.
