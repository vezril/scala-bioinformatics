## Purpose
Define the wright-fisher-expectation capability: the `WrightFisherExpectationProblem` validated parameter bundle (`n: Int`, `p: Vector[Probability]`, with bounds `1 ≤ n ≤ 1,000,000` and `|p| ≤ 20`) and its three-case error ADT, plus a new method on the existing `WrightFisher` object — `expectedFrequencies(problem): Vector[Double]` — computing `E[Bin(n, p)] = n · p` element-wise. Serves the Rosalind "Wright-Fisher's Expected Behaviour" (EBIN) problem and is the seventh algorithm in the `genetics` subdomain. The `WrightFisher` object now hosts two methods — `atLeast` (from `wright-fisher-genetic-drift`) and `expectedFrequencies` — making it the framework's first multi-method algorithm object; each method takes its own validated input bundle and the compiler routes calls by type.
