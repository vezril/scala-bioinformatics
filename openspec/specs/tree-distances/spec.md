# tree-distances Specification

## Purpose

Computes the number of edges between two labelled nodes in a parsed
`NewickTree` (Rosalind problem 30 — NWCK, "Distances in Trees"). Provides
a validated `NewickDistanceProblem` input bundle and the
`NewickDistance.between` BFS-based algorithm.
