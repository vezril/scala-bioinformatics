...
Solve Rosalind CONV by comparing two mass spectra: compute their spectral convolution `S₁⊖S₂` (the multiset of all pairwise differences) and report the largest multiplicity together with the absolute value of the maximizing shift. This capability covers validated `MassMultiset` inputs, the `SpectralConvolutionProblem` bundling type, the pure convolution algorithm, result formatting, and the `CONVProb` runner.
