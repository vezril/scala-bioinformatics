# genetic-character-table Specification

## Purpose

Computes the character-table representation of an aligned, characterizable
DNA matrix (Rosalind spec 33 — "Creating a Character Table from Genetic
Strings"). Each column with both possible symbols and a non-singleton
partition of the rows becomes a nontrivial character. Provides a validated
`GeneticCharacterTableProblem` input bundle and the `GeneticCharacterTable.compute`
