# noncrossing-matching Specification

## Purpose

Counts the noncrossing perfect matchings of basepair edges (`A`-`U` or
`C`-`G`) in the bonding graph of a balanced RNA string, modulo `1 000 000`
(Rosalind spec 35 — CAT, "Catalan Numbers and RNA Secondary Structures").
Provides a validated `NoncrossingMatchingProblem` input bundle (length cap
of 300 bp, balanced AU and CG checks) and the `NoncrossingMatching.count`
