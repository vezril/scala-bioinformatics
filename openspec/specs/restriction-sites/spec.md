...
Locate restriction sites in a DNA string (Rosalind REVP): validate a bounded DNA string, find every reverse palindrome of length 4 to 12 that equals its reverse complement, and report each one's 1-based position and length. This capability includes the validated `RestrictionSiteProblem` input, the `RestrictionSite`/`RestrictionSites` result types, the pure `locate` algorithm, result formatting, and the `REVPProb` runner.
