# spliced-motif Specification

## Purpose

Finds a subsequence-match of a target DNA pattern inside a source DNA string
(Rosalind spec 37 — SSEQ, "Finding a Spliced Motif"). Returns the
lexicographically-leftmost 1-indexed positions where the target's symbols
appear in order (not necessarily contiguously). Provides a validated
`SplicedMotifProblem` input bundle (length caps on both strings) and the
`SplicedMotif.find` `O(|s| + |t|)` greedy two-pointer algorithm.
