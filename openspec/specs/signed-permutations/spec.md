# signed-permutations Specification

## Purpose

Enumerates every signed permutation of length `n` (Rosalind spec — SIGN,
"Enumerating Oriented Gene Orderings"). A signed permutation pairs each of the
`n!` orderings of `{1, …, n}` with an independent `+` or `−` sign on every
element, yielding `n! · 2ⁿ` permutations. Provides a validated
`SignedPermutationProblem` input bundle (length 1 to 6), a `SignedPermutations`
result type that reports the count and renders the permutations, and the pure
`SignedPermutationEnumeration.enumerate` algorithm.
