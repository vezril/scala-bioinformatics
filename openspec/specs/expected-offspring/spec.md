## Purpose
Define the framework's population-level Mendelian expectation: the `CouplePopulation` domain type (validated counts of couples grouped by genotype pairing), the `CouplePopulationError` ADT, and the `ExpectedOffspring.dominantPhenotype(pop): Double` algorithm computing the expected number of dominant-phenotype offspring across the population assuming every couple produces exactly two offspring. Serves the Rosalind "Calculating Expected Offspring" problem and complements the spec-5 `MendelianInheritance` algorithm (probability-per-individual) with a parallel expected-count-per-couple view.
