...
This capability computes, for each GC-content in an array A, the expected number of times a motif s appears as a substring of a random DNA string of length n (Rosalind EVAL). For each entry it evaluates `B[i] = max(0, n - |s| + 1) · ∏ P(symbol | A[i])`, the product of the per-symbol probabilities of the motif under that GC-content.
