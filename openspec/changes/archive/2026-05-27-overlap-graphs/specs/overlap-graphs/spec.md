...
The system SHALL provide a `sealed trait OverlapLengthError` with case `final case class NonPositive(value: Int)`. The type SHALL reside in the `bio.domain.analysis` package.
...
- **WHEN** `OverlapGraph.adjacency` is called with `R1.dna = "ACG"` and `R2.dna = "GCA"` at k=1 (`R1` ends in `"G"`, matches `R2`'s prefix `"G"`)
